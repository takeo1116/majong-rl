...
### 0.1 パス記述ルール
- Git 管理下の文書は、必ず**リポジトリ相対パス**で書く
- **ローカル絶対パスは書かない**
- `runs/` 配下の run dir / `summary.json` / `run.log` などは、いつでも削除されうる前提で扱う
- `runs/` 配下を恒久的な参照先として runbook に書かない
- 実行結果の対応付けは、必要なら `experiments/exp_XXX/run_map.json` と `report.md` に転記して残す
...
- 新規実行するもの:
- 実データ確認:
  - 一時的なローカル成果物がまだ残っているか:
  - 残っていない場合、必要な値が `report.md` / `run_map.json` に転記されているか:
- 再実行が必要な理由:
...
- [ ] 絶対パスを書いていない
- [ ] `runs/` 配下を恒久参照先として書いていない
- [ ] 参照する既存 run の実データが残っているか、または必要値が `report.md` / `run_map.json` に転記済みかを確認した
